...
/** Deterministic PRNG so a seed reproduces the exact same dungeon. */
...
/** Builds DSL rows from the generator pools: fixed spine ends, shuffled
 *  middle rooms, pockets hung left or right per the seed. */
const generateBlueprintRows = (seed: string): readonly string[] | null => {
  const generator = ROOM_THEME_KIT.dungeonDsl.generator;
  ...
  const spine = shuffled(generator.spine, random);
  const pockets = shuffled(generator.pockets, random);
  const rows: string[] = [];
  ...
  for (let y = 0; y < blueprint.rows.length; y += 1) {
    ...
    for (let x = 0; x < row.length; x += 1) {
      ...
      rooms.set(id, {
        ...
        decorSeed: seededGridValue(x, y, row.charCodeAt(x) + 97 + (hashSeed(seed) % 89)),
        depth,
        enemyBudget,
...
function createDungeonBlueprint(seed: string): DungeonBlueprintSelection {
  const rows = normalizeBlueprintRows(generateBlueprintRows(seed) ?? DUNGEON_ROUTE_DSL);
